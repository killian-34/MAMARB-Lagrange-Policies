...
def runtimePlot(savestring, root, firstseed, num_trials, bfi, ssi, ignore_missing=False, logplot=False):
    ...
    if not logplot:
        outname = '%s_b%.1f_s%s_pl0%s_runtime.pdf'%(savestring,budget_frac,state_size,pl0)
    else:
        outname = '%s_b%.1f_s%s_pl0%s_runtime_logplot.pdf'%(savestring,budget_frac,state_size,pl0)
    # outname = '%s_b%.1f_s%s_runtime.pdf'%(savestring,budget_frac,state_size)
    # outname = 'debug_%s_b%.1f_s%s_runtime.pdf'%(savestring,budget_frac,state_size)
...
def groupedBarPlot(savestring, root, firstseed, num_trials, bfi, ssi):
    ...
    # outname = '%s_b%.1f_s%s_pl0%s_rewards.pdf'%(savestring,budget_frac,state_size,pl0)
    outname = '%s_b%.1f_s%s_rewards.pdf'%(savestring,budget_frac,state_size)
    # outname = 'debug_%s_b%.1f_s%s_rewards.pdf'%(savestring,budget_frac,state_size)
    plt.savefig(outname)
    # plt.show()
